...
int main(void)
{
    int i,j=1,k,l;
    bool found=false;
    Graph GM(2),HM(2),GH,HG,PM,MP,M,N;
    ...
    property_map<Graph,edge_name_t>::type
    N_ename = get(edge_name,N);

    property_map<Graph,edge_name_t>::type
    M_ename = get(edge_name,M);

    GEI ei,ei_end,fi,fi_end;

    ofstream grid("gridOneOneTwo.txt"),loSEs("listOfSEsOneOneTwo.txt");

    add_edge(0,1,string("a"),GM);
    add_edge(1,0,string("b"),GM);
    add_edge(1,1,string("c"),GM);
    add_edge(1,1,string("d"),GM);
    ...
    add_edge(1,0,string("v"),HM);
    add_edge(1,1,string("w"),HM);
    add_edge(1,1,string("x"),HM);
	...
	else
	  {
        ...
        for(k=-1; k>-3;k--)
        {
            for(l=1; l<4; l++)
            {
                ...
                if(found){
                    grid << "PRINTING IJ CONJUGACY FOR SE" << j << " where k = " <<  k  << " and l = " << l << endl;
                    PrintFullTextileInfo(Tkl,grid);
                    PrintFullTextileInfo(Tconj,grid);
                    stringstream filename;
                    filename << "CONJUGACY" << j << k << l;
